        ...
        <div class="content">
            ...
            <div class="block block-rounded">
                <div class="block-header block-header-default">
                    <h3 class="block-title">Edit Passages</h3>
                </div>
                <div class="block-content block-content-full">
                    <form action="{{route('passages.update',['passage'=> $passage->id])}}" method="POST" >
                        @csrf
                        @method('PUT')
                        <div class="row py-3">
                            <div class="col-md-12">
                                ...
                                <div class="mb-2">
                                    ...
                                    <input type="text" class="form-control form-control-lg form-control-alt {{$errors->has('title') ? 'is-invalid' : ''}}"
                                           id="title" name="title" placeholder="Title" value="{{ $passage->title }}" required>
                                    @error('name')
                            ...
                            {{ $passage->description }}</textarea>
                                ...
                                <div class="mb-2">
                                    ...
                                    {{$errors->has('description') ? 'is-invalid' : ''}}" id="description" name="description"
                                        ...
                                        <option value="PSAT" @php if($passage->type == 'PSAT'){ echo 'selected="selected"';} @endphp >PSAT</option>
                                        <option value="SAT" @php if($passage->type == 'SAT'){ echo 'selected="selected"';} @endphp>SAT</option>
                                        <option value="ACT" @php if($passage->type == 'ACT'){ echo 'selected="selected"';} @endphp>ACT</option>
                                    </select>
                                    @error('type')
                                    <div class="invalid-feedback">{{$message}}</div>
                                    @enderror
