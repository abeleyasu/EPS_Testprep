...
<main id="main-container">
    ...
    <!-- Hero Content -->
    <div class="bg-image"   >
        <div class="bg-primary" style="background:url({{ asset('/public/Image/'.$module->coverimage) }});background-repeat: no-repeat;background-position: center; background-size: cover">
					...
					@foreach($getModules as $mokey => $getModule)
						@if ($module->id == $getModule->id)
							...
							@if ($lastItem == $mokey)
                                ...
                                @endphp   
                                @if (!empty($allMilestone[$nextMilestone]))
                                    ...
                                    {{-- <a href="{{ route('milestone.detail',['milestone' => $getModules[0]->milestone_id]) }}" class="btn w-25 btn-alt-success btnminwidth">
                                        <i class="fa fa-fw fa-eye me-1 opacity-50"></i> Next Milestone
                                    </a> --}}
                                @endif
